...
def evaluate(cvae, testLoader, exDir, e=1, classifier=None):  # e is the epoch
    ...
    if cvae.useCUDA:
        z = torch.randn(xTest.size(0), opts.nz).cuda()
    else:
        z = torch.randn(xTest.size(0), opts.nz)

    ySmile = torch.Tensor(yTest.size()).fill_(1).type_as(yTest)
    samples = cvae.decode(ySmile, z).cpu()
    save_image(samples.data, join(exDir, 'smile_epoch' + str(e) + '.png'))

    yNoSmile = torch.Tensor(yTest.size()).fill_(0).type_as(yTest)
    ...
    if e == 'evalMode' and classer is not None:
        classer.eval()
        yPred0 = classer(rec0)
        y0 = torch.LongTensor(yPred0.size()).fill_(0).type_as(yTest)
        class0 = binary_class_score(yPred0, y0, thresh=0.5)
        yPred1 = classer(rec1)
        y1 = torch.LongTensor(yPred1.size()).fill_(1).type_as(yTest)
...
if __name__ == '__main__':
    ...
    ####### Define optimizer #######
    optimizerCVAE = optim.RMSprop(cvae.parameters(), lr=opts.lr,
                                  weight_decay=opts.weightDecay)  # specify the params that are being upated
    ...
    for e in range(opts.maxEpochs):
        ...
        for i, data in enumerate(trainLoader, 0):
            ...
            ###############################################
            ###############################################
            #Why not cvae.forward?
            outRec, outMu, outLogVar, predY = cvae.forward(x)
            ###############################################
            ###############################################
            ...
            if i % 100 == 1:
                ...
                (e, i, epochLoss / i, epochLoss_dis / i, epochLoss_gen / i, epochLoss_bce / i, epochLoss_kl / i,
                 epochLoss_aux / i, time() - TIME))
