...
def label_switch(x, y, cvae, exDir=None):
    ...
    ySmile = torch.eye(2)[torch.LongTensor(np.ones(y.size(0), dtype=int))].type_as(z)
    smileSamples = cvae.decode(ySmile, z).cpu()

    yNoSmile = torch.eye(2)[torch.LongTensor(np.zeros(y.size(0), dtype=int))].type_as(z)
...
def rand_one_hot(noSamples, noLabels, useCUDA):
    y = torch.eye(noLabels)[torch.LongTensor(np.random.randint(0, noLabels, noSamples, dtype=int))].type(
        torch.FloatTensor)
    if useCUDA:
        return y.cuda()
    else:
