...
from function import make_new_folder, plot_losses, vae_loss_fn, save_input_args, \
    sample_z, class_loss_fn, label_switch, plot_norm_losses  # , one_hot
...
if __name__ == '__main__':
    ...
    for e in range(opts.maxEpochs):
        ...
        for i, data in enumerate(trainLoader, 0):
            ...
            fakeLabel = torch.Tensor(pXreal.size()).zero_().type_as(pXreal)
            realLabel = torch.Tensor(pXreal.size()).fill_(1).type_as(pXreal)
            ...
            if i % 100 == 0:
                ...
                (e, i, epochLoss / i, epochLoss_bce / i, opts.alpha * epochLoss_kl / i, epochLoss_gen / i,
                 epochLoss_dis / i, \
        ...
        if e % 10 == 0:
            ...
            ySmile = torch.eye(2)[torch.LongTensor(np.ones(yTest.size()).astype(int))].type_as(z)
            samples = cvae.decode(ySmile, z).cpu()
            save_image(samples.data, join(exDir, 'smile_epoch' + str(e) + '.png'))

            yNoSmile = torch.eye(2)[torch.LongTensor(np.zeros(yTest.size()).astype(int))].type_as(z)
